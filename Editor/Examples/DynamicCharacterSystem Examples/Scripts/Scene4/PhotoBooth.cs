	...
	public class PhotoBooth : MonoBehaviour
	{
		...
		public void TakePhotos()
		{
			if (doingTakePhoto == false)
			{
				...
				{
					Dictionary<string, List<UMATextRecipe>> recipesToPhoto = UMAContextBase.Instance.GetRecipes(avatarToPhoto.activeRace.name);
		...
		IEnumerator TakePhotosCoroutine(Dictionary<string, List<UMATextRecipe>> recipesToPhoto)
		{
			...
			if (!basePhotoTaken)
			{
				...
				{
					AddUnderwearToAvatar(recipesToPhoto);
				}
				...
				yield return new WaitUntil(() => canTakePhoto);
		...
		private void AddUnderwearToAvatar(Dictionary<string, List<UMATextRecipe>> recipesToPhoto)
		{
			foreach (KeyValuePair<string, List<UMATextRecipe>> kp in recipesToPhoto)
			{
				if (kp.Key.IndexOf("Underwear") > -1 || kp.Key.IndexOf("underwear") > -1)
